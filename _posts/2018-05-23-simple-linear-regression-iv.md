...
Note that $$M = X'(X'X)^{-1}X$$ is symmetric -- $$(X'(X'X)^{-1}X)' = X'(X'X)^{-1}X$$ -- so 
...
**Proof:**  First, recognize that $$Y'AY$$ is a scalar. Hence $$tr(Y'AY) = Y'AY$$. Second, note
...
Or in other words, $$\frac{1}{n -p} SSE$$ is an unbiased estimator of $$\sigma^2$$. 